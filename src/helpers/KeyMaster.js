...
const _createAccountCreationKey = async recoveryBytes => {
  const rootPrivateKey = await Keyaddr.newKey(recoveryBytes)
...
const _createAccount = async (
  ...
  const privateKeyForAddress = await Keyaddr.child(
  ...
  const publicKey = await Keyaddr.toPublic(privateKeyForAddress)

  const newKey = createKey(privateKeyForAddress, publicKey, childPath)
  wallet.keys[privateKeyHash] = newKey

  const address = await Keyaddr.ndauAddress(publicKey)
...
const createAccountFromPath = async (
  ...
  if (rootPrivateKey) {
    ...
    privateDerivedKey = await Keyaddr.deriveFrom(
      rootPrivateKey,
      '/',
      derivedPath
    )
  ...
  const publicKey = await Keyaddr.toPublic(privateDerivedKey)
