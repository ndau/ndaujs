...
describe('Multisafe', () => {
  /* const wordsToBytes = sinon.spy(Keyaddr, 'wordsToBytes')
  wordsToBytes.mockReturnValue(recoveryPhraseBytes)
  const newKey = sinon.spy(Keyaddr, 'newKey')
  newKey.mockReturnValue(initialPrivateKey)

  const hardenedChild = sinon.spy(Keyaddr, 'hardenedChild')
  hardenedChild
  ...
  const child = sinon.spy(Keyaddr, 'child')
  child
  ...
  const ndauAddress = sinon.spy(Keyaddr, 'ndauAddress')
  ndauAddress
  ...
  const deriveFrom = sinon.spy(Keyaddr, 'deriveFrom')
  deriveFrom
  ...
  const toPublic = sinon.spy(Keyaddr, 'toPublic')
  toPublic
...
*/
  it('setupNewUser creates a MultiSafe and we can then retrieve with password', async () => {
    ...
    expect(user).to.equal(userGettingCreated)
  })
  ...
  it('setupTestUser creates a MultiSafe and we can then retrieve with recovery phrase', async () => {
    ...
    expect(theNewUser).to.equal(newUser)
  })
  ...
  it('setupTestUser creates a MultiSafe, retrieve with recovery and then resetPassword', async () => {
    ...
    expect(userFromNewPassword).to.equal(userGettingCreated)
    ...
    expect(userFromNewPassword1).to.equal(userGettingCreated)
