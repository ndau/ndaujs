import { NotifyTransaction } from '../../src/transactions/NotifyTransaction'
...
describe('Notify Transaction', () => {
  ...
  it('creation of a notify transaction', async () => {
    ...
    expect(createdNotifyTransaction).to.equal(theNotifyTransaction)
  })

  it('notify fails if no sequence', async () => {
  ...
  it('creation and signature of a notify transaction and make sure that we do NOT resequence if create is called again', async () => {
    MockHelper.mockAccountAPI(
      data.testSingleAddressData,
      'tnaq9cjf54ct59bmua78iuv6gtpjtdunc78q8jebwgmxyacb'
    )
    ...
    let createdNotifyTransaction = await notifyTransaction.create()
    await notifyTransaction.sign()

    expect(createdNotifyTransaction).to.equal(theNotifyTransaction)
    expect(createdNotifyTransaction.sequence).to.equal(
      theNotifyTransaction.sequence
    )
    ...
    createdNotifyTransaction = await notifyTransaction.create()
    await notifyTransaction.sign()
