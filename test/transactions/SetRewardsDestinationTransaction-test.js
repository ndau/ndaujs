import { SetRewardsDestinationTransaction } from '../../src/transactions/SetRewardsDestinationTransaction'
...
describe('SetRewardsDestination Transaction', () => {
  before(() => {
  ...
  it('creation of a setRewardsDestination transaction', async () => {
    const theSetRewardsDestinationTransaction = {
      destination: 'ndarw5i7rmqtqstw4mtnchmfvxnrq4k3e2ytsyvsc7nxt2y7',
    ...
    const createdSetRewardsDestinationTransaction = await setRewardsDestinationTransaction.create()
    expect(createdSetRewardsDestinationTransaction).to.equal(
      theSetRewardsDestinationTransaction
    )
  })

  it('setRewardsDestination fails if no sequence', async () => {
    ...
    try {
      ...
      await setRewardsDestinationTransaction.create()
  ...
  it('failure of any transaction around sign', async () => {
    ...
    try {
      ...
      await setRewardsDestinationTransaction.sign()
    } catch (error) {
  ...
  it('failure of any transaction around prevalidate', async () => {
    ...
    try {
      ...
      await setRewardsDestinationTransaction.prevalidate()
    } catch (error) {
  ...
  it('failure of any transaction around submit', async () => {
    ...
    try {
      ...
      await setRewardsDestinationTransaction.submit()
    } catch (error) {
      expect(error.toString()).to.equal('Error: error being sent')
    }
  })

  it('creation and signature of a setRewardsDestination transaction and make sure that we do NOT resequence if create is called again', async () => {
    MockHelper.mockAccountAPI(
      data.testSingleAddressData,
      'tnaq9cjf54ct59bmua78iuv6gtpjtdunc78q8jebwgmxyacb'
    )
    ...
    let createdSetRewardsDestinationTransaction = await setRewardsDestinationTransaction.create()
    await setRewardsDestinationTransaction.sign()

    expect(createdSetRewardsDestinationTransaction).to.equal(
      theSetRewardsDestinationTransaction
    )
    expect(createdSetRewardsDestinationTransaction.sequence).to.equal(
      theSetRewardsDestinationTransaction.sequence
    )
    ...
    createdSetRewardsDestinationTransaction = await setRewardsDestinationTransaction.create()
    await setRewardsDestinationTransaction.sign()
