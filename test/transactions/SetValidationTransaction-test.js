...
describe('SetValidation Transaction', () => {
  before(() => {
  ...
  it('creation of a setValidation transaction', async () => {
    ...
    expect(createdSetValidationTransaction).to.equal(theSetValidationTransaction)
  })
  ...
  it('creation a transaction yeilds new instance', async () => {
    ...
    expect(createdSetValidationTransaction1).to.equal(
      theSetValidationTransaction
    )
  ...
  it('setValidation fails if no sequence', async () => {
  ...
  it('failure of any transaction around sign', async () => {
    ...
    try {
      ...
      const createdSetValidationTransaction = await setValidationTransaction.create()
      expect(createdSetValidationTransaction).to.equal(
        theSetValidationTransaction
      )
  ...
  it('creation and signature of a SetValidation transaction and make sure that we do NOT resequence if create is called again', async () => {
    MockHelper.mockAccountAPI(
      data.testSingleAddressData,
      'tnaq9cjf54ct59bmua78iuv6gtpjtdunc78q8jebwgmxyacb'
    )
    ...
    expect(createdSetValidationTransaction).to.equal(theSetValidationTransaction)
    expect(createdSetValidationTransaction.sequence).to.equal(
